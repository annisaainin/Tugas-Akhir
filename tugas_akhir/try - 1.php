										...
										<div class="select-wrap">
											<div class="icon"><span class="fa fa-chevron-down"></span></div>
												<select name="" id="filter" class="form-control">
													...
													<option value="kafe dan eatery">kafe dan eatery</option>
...
<?php
include "koneksi.php";
$query =  mysqli_query($koneksi, "SELECT CONCAT('[',latitude,',',longitude,']') as loc, nama as title, nama, gambar, kategori, jam_buka, rate, kisaran_harga, latitude, longitude, redirect_link FROM `tb_lokasi`;"); 
$data = []; 
...
<script>
var map = L.map('map').setView([-7.768738726985844, 113.19529509574068], 13);
...
markerLayer.clearLayers(); // Clear existing markers
	data.forEach(function(d) {
        ...
        let jarak = map.distance(position, [d.latitude, d.longitude]);
		...
		// if (jarak < radius){
			...
			if (selectedValue === 'all' || d.kategori === selectedValue) {
				...
				marker.bindPopup(title +'<br>' + d.kategori + '<br>' + d.jam_buka + '<br>' + d.rate + 
				'<br>' + jarak.toFixed(2)  + '<br> <a href="' + d.redirect_link + '" target="_blank">Details</a>');
